...
### User Story 3 - 每镜按分镜换一套字幕身份 (P1)

合成网页对照官方字幕组件库：全屏砸字、杂志双字体、擦除揭示、字重切换。同一镜只保一种身份，换镜才换风格。选法看图片分镜（景别、场景、运动），不是五镜同一套。屏幕字仍是海报贴边标题。

**Why this priority**: 用户要求按 Catalog 学习，并按图片分镜设计不同的合成网页。

**Independent Test**: 兜底稿在多样分镜上出现不少于三种字幕身份；单测锁定选片规则。

**Acceptance Scenarios**:

1. **Given** 分镜有特写/夜景横移/拉远/砸切，**When** 写合成稿，**Then** 各镜字幕身份不同（杂志体 / 擦除 / 字重 / 全屏砸字），屏幕字仍是海报标题
2. **Given** 同一镜，**When** 出字，**Then** 不把两套字幕身份叠在这一镜里
...
- **FR-006**: C 端不写厂商名
- **FR-007**: 字幕身份 MUST 按图片分镜逐镜选择；同一镜不得混用两套身份；换镜才换风格
