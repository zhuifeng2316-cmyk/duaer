...
助手回复里若带出目录内中文名，用对应故事静帧做样式预览；可一键用到全片，或按钩子/中段/收尾用到那一镜（画面已可改字幕时）。右侧内容区同步能看到预览卡。
...
- **SC-001**: 单测锁定目录进 prompt、模型名配置、mock 回复含真实中文名与镜上下文
- **SC-002**: 接口可对某条口播发消息并拿到回复
- **SC-003**: 浏览器左侧与右侧内容区都能看到叠在故事片上的样式预览，并可分镜/全片应用
