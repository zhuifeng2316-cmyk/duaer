...
2. 用户问组件/画面块/叠层/动效组件时，只从组件目录挑中文名，同样按钩子/中段/收尾与镜号写。当前题材优先；若适合讲清楚信息，也可从「可独立用」里挑代码、图表、界面等独立画面块。
3. 一次推荐 3～6 个即可；提到名称时必须用目录原词，方便作者点选预览。可混合题材叠层与独立画面块。
...
export function mockTalkAssistReply(userText: string): string {
  ...
  if (askGraphic) {
    return [
      ...
      "- 漏光：情绪段加一层暖光。",
      "- 手写标题：要点写成手写感。",
      "- 代码对照：若要讲清楚对比，可叠独立代码块。",
      "",
      "### 收尾",
      "- 胶片颗粒：收束成片气质。",
      "- 故障字：如果要更冲一点再用。",
      "",
      "独立画面块还可试「柱图表赛跑」「对话露出」这类。",
